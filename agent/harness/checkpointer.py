...
from agent.harness.config import CheckpointBackend, CheckpointConfig

logger = logging.getLogger(__name__)


# ---------------------------------------------------------------------------
# .env 加载：checkpointer 可能被 langgraph.json 独立加载（早于 model_config 的
# load_dotenv），这里先加载根目录 .env，保证 os.getenv 能读到配置。
# 默认不覆盖已有环境变量（shell export 优先），重复调用无副作用。
# ---------------------------------------------------------------------------
