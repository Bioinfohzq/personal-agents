...
>
> **术语约定**:本项目中的"**知识库**"指所有个人数据构成的统一向量检索池子(包含知识记录、长期记忆、命令手册等,底层对应embeddings表),是产品的核心概念;前端「知识记录」页面对应的数据类型叫"**知识记录**"(knowledge_items 表),只是知识库中的一种数据类型,不能单独称为"库"。前端导航和按钮中统一使用"知识记录",不使用"知识条目"或"知识库"指代单条记录。

---

## 一、产品愿景

一句话:**"我的所有工作记录、知识、经验,全部集中在这里;我和 AI 的每一次有价值对话,都能一键沉淀为知识记录;我问任何问题时,AI 首先检索我的知识库,结合我过往经验再回答。"**
...
### 场景 1:知识沉淀(手动)
我有一份技术方案的 Markdown 文档,直接在「知识记录」页面上传,选择分类保存为知识记录。之后我问相关问题,AI 能直接引用这份文档的内容。

### 场景 2:对话中沉淀(一键保存)
我和 AI 讨论一个架构问题,AI 给出了一段很有价值的总结。我点那条消息旁边的「📥 存入知识记录」按钮,选择分类,这段对话就自动成为一条知识记录,带上对话上下文链接。
...
### 场景 4:RAG 增强问答
我提问时,AI 自动检索我的知识库(知识记录+记忆+历史对话等),把相关内容塞进上下文,回答时会引用"根据你在《XX 笔记》中的记录..."而不是空口胡说。
...
| # | 能力 | 状态 | 说明 | 相关模块 |
|---|------|------|------|---------|
| A1 | 知识记录 CRUD(增删改查) | ✅ | 完整的 REST API + 前端表单 | backend/internal/knowledgebook, web/.../KnowledgebookPage |
...
| A11 | **知识记录关联/反向链接** | ❌ | 记录间互相引用、"相关知识"推荐 | 待建 |
...
| # | 能力 | 状态 | 说明 | 相关模块 |
|---|------|------|------|---------|
| B1 | 基础对话 UI(流式输出+工具状态指示) | ✅ | LangGraph SDK + SSE;流式等待状态气泡:思考阶段显示"正在思考...",工具调用阶段自动切换为对应中文提示("正在检索知识库...""正在回忆相关内容..."等)+图标+三点动画,工具结果返回后显示轻量"正在整理回复..."指示器 | web/.../Chat, agent/graph.py, web/src/pages/ChatPage.tsx, web/src/components/Chat/MessageList.tsx |
| B2 | 多会话管理 | ✅ | 会话列表、切换、删除 | web/.../ChatPage |
| B3 | 智能体工具调用(文件/Shell/搜索/计算器等) | ✅ | MCP + builtin tools | agent/tools/ |
| B4 | **对话消息一键存为知识记录** | ✅ | 消息气泡悬浮时显示「📥 存入知识记录」按钮(仅user/agent消息,tool消息不显示),弹出Modal选择分类+编辑标题/标签/摘要/内容,提交后创建知识记录并自动生成向量;知识记录标记source_thread_id/source_msg_id/source_role记录来源;保存成功顶部toast提示 | web/src/components/Chat/MessageBubble.tsx, web/src/components/Chat/SaveToKnowledgeModal.tsx, backend/internal/knowledgebook/ |
| B5 | **对话片段全局检索** | ❌ | 历史消息全文+语义搜索,点击跳转到对话位置 | 待建:messages 表 + 搜索接口 |
| B6 | **RAG 注入(回答前检索知识库/记忆)** | ✅ | Agent 通过 `search_knowledge_base`(全局知识库:知识记录+命令+记忆) 和 `recall_memory`(仅记忆库) 两个工具自主判断是否检索,命中内容会引用来源标题;后端提供 `/api/v1/internal/search`(internal key鉴权)供Agent调用,`/api/v1/search`(JWT鉴权)供前端调用;全局搜索已支持知识记录+命令+记忆三类语义检索(UNION ALL合并) | agent/tools/builtin/knowledge.py, agent/tools/builtin/memory.py, agent/harness/prompts/system.py, backend/internal/search/handler.go |
...
| B8 | **记忆管理界面** | ✅ | 侧边栏底部「我的空间」弹出菜单中「长期记忆」入口;列表支持topic分类筛选/关键词搜索/显示归档切换/按重要度+时间排序;弹窗新建/编辑记忆(选择topic、填写标题、选择1-5重要度、填写内容、设置激活状态);归档/恢复/删除操作(删除带二次确认) | web/src/components/Memorybook/MemorybookPage.tsx, web/src/api/memorybook.ts, web/src/types/memorybook.ts, web/src/router/index.tsx, web/src/components/Sidebar/Sidebar.tsx |
...
- **不要记**:临时问题、一次性任务、闲聊问候;情绪表达、不确定的猜测;可通过工具实时查到的信息;知识库已有正式知识记录的内容
...
| C2 | **顶部全局搜索框** | ❌ | Header 中的统一搜索入口,跨模块聚合结果(知识记录+对话+命令+记忆) | 待建:Header + 统一 search 接口 |
...
| D3 | **知识记录自动向量化** | ✅ | 知识记录创建/更新时在事务内自动计算并存储embedding,更新时ON CONFLICT幂等覆盖,删除时同事务删向量(事务双删);向量生成失败不阻塞主流程 | backend/internal/knowledgebook/store.go |
| D3b | **命令条目自动向量化** | ✅ | 与D3对称:命令(commands表)Create/Update/Delete时同样在事务内自动处理向量,复用embeddings表(source_type='command');全局搜索通过UNION ALL合并知识记录+命令结果 | backend/internal/commandbook/store.go, backend/internal/search/handler.go |
| D4 | **消息向量化** | ❌ 不自动做 | **采用方案B**:对话消息本身不自动向量化(避免噪音);只有用户点击「📥存入知识记录」(B4)后,消息内容才会作为知识记录入库并生成向量。近期上下文检索依赖会话历史本身(滑动窗口),不走向量库 | — |
| D5 | **记忆向量化** | ✅ | 长期记忆(memories表)Create/Update/Delete时自动在事务内处理embedding,复用embeddings表(source_type='memory');核心记忆(importance≥4)通过CoreMemoryMiddleware在对话启动时预注入;非核心记忆通过recall_memory工具语义检索;全局搜索已支持memory类型;前端工具调用状态指示器会实时显示"正在检索知识库..."/"正在回忆相关内容..."等提示 | backend/internal/memorybook/store.go, agent/harness/memory_middleware.py, agent/tools/builtin/memory.py, backend/internal/search/handler.go, web/src/components/Chat/MessageList.tsx |
...
| E7 | **数据导入/导出(Markdown/JSON)** | ❌ | 知识库数据备份与迁移(知识记录+记忆+命令) | 待建 |
| E8 | **移动端** | ❌ | 远期 | - |
...
- B4 对话消息一键存为知识记录 → C2 顶部全局搜索框(关键词) → B5 对话片段搜索
- **效果**:聊天里有价值的内容能存下来,全局能搜到,这就已经比豆包强了

### 阶段 2:RAG 增强问答 ✅ 已完成
- D1/D2/D3 embedding + 向量存储 → A10 知识记录语义检索 → B6 RAG 注入 agent
- **效果**:提问时 AI 会自主判断是否需要检索知识库,命中内容时引用你的知识记录回答
...
### 阶段 4:体验打磨与桌面端(下一阶段)
- C2 顶部全局搜索框(前端) → C3/C4 混合检索与结果分组 → A11/A12 知识关联与附件 → E5/E6 桌面端完善与本地 embedding
...
|-------------|--------|------|---------|
| `knowledge` | knowledge_items | 知识记录(手动录入/对话沉淀/文档上传) | ✅ 已实现自动向量化 |
| `command` | commands | 命令手册条目 | ✅ 已实现自动向量化 |
| `memory` | memories | 长期记忆(偏好/教训/约定/事实/沟通风格) | ✅ 已实现自动向量化 |
| `message` | messages(待建) | 对话消息片段 | ❌ 待建(不自动做,B4一键存知识记录已满足需求) |
...
3. **新增 `messages` 相关表**存储对话历史(当前由 LangGraph checkpointer 管理,语义检索需要独立表;近期不做,B4一键存知识记录已覆盖主要场景)
